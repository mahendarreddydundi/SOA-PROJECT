...
@Component public class JwtGatewayFilter implements GlobalFilter, Ordered {
 ...
 @Override public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain){String path=exchange.getRequest().getURI().getPath(); if(path.startsWith("/auth/")||path.equals("/actuator/health"))return chain.filter(exchange); String header=exchange.getRequest().getHeaders().getFirst("Authorization"); if(header==null||!header.startsWith("Bearer "))return reject(exchange); try{Jwts.parser().verifyWith(key).build().parseSignedClaims(header.substring(7)); return chain.filter(exchange);}catch(RuntimeException ex){return reject(exchange);}}
