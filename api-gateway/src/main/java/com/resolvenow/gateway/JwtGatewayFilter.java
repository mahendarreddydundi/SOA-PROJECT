...
@Component public class JwtGatewayFilter implements GlobalFilter, Ordered {
 ...
 @Override public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain){String path=exchange.getRequest().getURI().getPath(); String method=exchange.getRequest().getMethod().name(); if(method.equals("OPTIONS")||path.startsWith("/auth/")||path.equals("/actuator/health"))return chain.filter(exchange); String header=exchange.getRequest().getHeaders().getFirst("Authorization"); if(header==null||!header.startsWith("Bearer "))return reject(exchange); try{String subject=Jwts.parser().verifyWith(key).build().parseSignedClaims(header.substring(7)).getPayload().getSubject(); if(subject==null||subject.isBlank())return reject(exchange); return chain.filter(exchange);}catch(RuntimeException ex){return reject(exchange);}}
