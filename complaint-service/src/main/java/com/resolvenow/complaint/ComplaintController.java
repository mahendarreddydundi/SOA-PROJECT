...
import org.springframework.web.bind.annotation.*; import jakarta.validation.Valid; import java.util.*;
@RestController @RequestMapping("/complaints") public class ComplaintController { private final ComplaintService service; public ComplaintController(ComplaintService service){this.service=service;}
 @GetMapping public List<Complaint> all(){return service.all();} @GetMapping("/{id}") public Complaint one(@PathVariable Long id){return service.one(id);} @PostMapping public Complaint create(@Valid @RequestBody Complaint complaint){return service.create(complaint);}
